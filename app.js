...
const OTHER_FIELD_LISTS = { country: () => Store.countries, customer: () => Store.customers, equipment: () => Store.equipment, machine: () => Store.machines, leave: () => Store.leaveOptions };
...
function leaveOptions(sel)    { return otherFieldOptions('leave', sel); }
...
function buildRow(e, ts, editable) {
  ...
  if (editable && usesDayGrid()) {
    ...
    tr.innerHTML = `
      ...
      <td class="tsx-ctry-cell" id="field-cell-leave-${e.id}">${otherFieldCellHtml('leave', ts, e)}</td>
...
const OTHER_FIELD_PLACEHOLDERS = { country: 'Type country', customer: 'Type customer', equipment: 'Type equipment', machine: 'Type machine number', leave: 'Type leave type' };
const OTHER_FIELD_FLAGS = { country: '_countryOther', customer: '_custOther', equipment: '_equipmentOther', machine: '_machineOther', leave: '_leaveOther' };
...
function reviewRowHtml(e) {
  ...
  return `<tr class="${cls}">
    ...
    <td><select class="ts-input tsx-ctry" data-e="${e.id}" data-f="leave">${leaveOptions(e.leave)}</select></td>
